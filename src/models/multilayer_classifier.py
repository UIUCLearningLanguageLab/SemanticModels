...
class NumpyMultilayerClassifier(classifier.Classifier):
    ...
    def __init__(self, dataset, all_folds=True, verbose=False,
        ...
        print("\nTraining Multilayer Classifier of Size {}-{}-{} on {} for {} epochs".format(self.input_size, self.hidden_size,
                                                                                                    self.output_size, self.dataset.dataset_name, self.num_epochs))


        self.create_model_name('mlc')
    ...
    def init_weights(self):
        for i in range(self.dataset.num_folds):
            ...
            self.y_h_weight_list.append(np.random.normal(0, self.weight_init_stdev, [self.output_size, self.hidden_size]))
            self.h_bias_list.append(np.random.normal(0, self.weight_init_stdev, [self.hidden_size]))
            self.h_x_weight_list.append(np.random.normal(0, self.weight_init_stdev, [self.hidden_size, self.input_size]))
